...
class AppPermissionsStatus {
  ...
  List<String> get missingRecommendedPermissions {
    final missing = <String>[];
    if (!bluetoothGranted) missing.add('Bluetooth');
    if (!isNotificationGranted && Platform.isAndroid) missing.add('Notifications');
...
class PermissionService {
  ...
  Future<bool> _checkBluetoothPermission() async {
    try {
      ...
      // flutter_blue_plus handles this internally, but we can check via adapter state
      final state = await FlutterBluePlus.adapterState.first.timeout(
        const Duration(seconds: 2),
        onTimeout: () => BluetoothAdapterState.unknown,
      );
      
      // If unauthorized, permission is denied
      if (state == BluetoothAdapterState.unauthorized) {
        return false;
      }
      
      // Also check explicit permissions on Android
      if (Platform.isAndroid) {
        final scanStatus = await Permission.bluetoothScan.status;
        final connectStatus = await Permission.bluetoothConnect.status;
        
        // On Android 12+, both need to be granted
        // On older Android, bluetoothScan might not exist - check bluetooth permission instead
        if (scanStatus == PermissionStatus.permanentlyDenied ||
            connectStatus == PermissionStatus.permanentlyDenied) {
          return false;
        }
        
        // Try to check if we can actually scan
  ...
  Future<bool> requestBluetoothPermission() async {
    ...
    try {
      if (Platform.isAndroid) {
        ...
        // iOS - try to trigger BLE permission by starting a scan
  ...
  Future<PermissionStatus> requestNotificationPermission() async {
    ...
    try {
      ...
      debugPrint('[PermissionService] Notification permission request error: $e');
      return PermissionStatus.denied;
    }
  }
  ...
  Future<bool> requestBatteryOptimizationExemption() async {
    debugPrint('[PermissionService] Requesting battery optimization exemption...');
    
    if (Platform.isAndroid) {
      try {
        return await _foregroundService.requestDisableBatteryOptimization();
      } catch (e) {
        debugPrint('[PermissionService] Error requesting battery exemption: $e');
        return false;
      }
    }
    return true;
